  ...
  return (
    <GlowCard className="p-6 lg:col-span-2">
      <div className="mb-6 flex items-center justify-between gap-4">
        ...
        <Link href={`/app/${slug}/new-project`} className="rounded-xl bg-white px-4 py-2 text-xs font-bold text-[#05070d] transition hover:bg-blue-50">New Project</Link>
      </div>
      {projects && projects.length > 0 ? (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {projects.map((project) => (
            <Link key={project.id} href={`/app/${slug}/projects/${project.id}`} className="block rounded-2xl border border-white/8 bg-[#070b14]/80 p-4 transition hover:-translate-y-0.5 hover:border-blue-400/30 hover:bg-blue-500/5">
              <div className="mb-4 flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1"><div className="truncate text-base font-bold text-white">{project.name}</div><div className="mt-1 font-mono text-xs text-blue-300">${project.ticker}</div></div>
                <StatusChip tone={STATUS_TONE[project.status] ?? "blue"}>{STATUS_LABEL[project.status] ?? project.status.replace("_", " ")}</StatusChip>
              </div>
              <div className="flex items-center justify-between border-t border-white/8 pt-3 text-[10px] uppercase tracking-widest text-slate-600"><span>{project.chain}</span><span>Open launch room</span></div>
            </Link>
          ))}
        </div>
      ) : (
