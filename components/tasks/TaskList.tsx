...
export function TaskList({ tasks }: TaskListProps) {
  ...
  return (
    <div className="space-y-2">
      ...
      {localTasks.map((task) => (
        <div key={task.id} className="p-4 bg-[#070b14] rounded-2xl border border-white/6 hover:border-white/10 transition-colors flex gap-3">
          <PriorityIndicator priority={PRIORITY_MAP[task.priority] || "medium"} variant="bar" />
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3 mb-2">
              ...
              <StatusChip tone={STATUS_TONE[task.status] || "blue"}>{STATUS_LABELS[task.status]}</StatusChip>
            </div>
