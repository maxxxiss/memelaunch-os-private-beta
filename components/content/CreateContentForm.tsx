...
export function CreateContentForm({ workspaceId, workspaceSlug, projectId }: CreateContentFormProps) {
  ...
  return (
    <GlowCard className="p-5">
      ...
      <h3 className="text-lg font-bold text-white mb-2">Schedule Content</h3>
      <p className="text-xs text-slate-400 mb-5 leading-5">Plan posts for X, Telegram, Discord, or your website before launch.</p>
      <form action={handleSubmit} className="space-y-4">
        {error && <ErrorMessage title="Content creation failed" message={error} onRetry={() => setError(null)} />}
        ...
        <div>
          ...
          <input id="title" name="title" type="text" required maxLength={200} className="w-full px-3 py-2 bg-[#0b1020] border border-white/10 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors" placeholder="e.g. Launch announcement thread" />
        </div>
        <div>
          <label htmlFor="platform" className="block text-sm font-medium text-slate-300 mb-1">Platform</label>
          <select id="platform" name="platform" className="w-full px-3 py-2 bg-[#0b1020] border border-white/10 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors">
        ...
        <div>
          ...
          <input id="scheduledAt" name="scheduledAt" type="datetime-local" className="w-full px-3 py-2 bg-[#0b1020] border border-white/10 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors" />
        ...
        <div>
          ...
          <textarea id="notes" name="notes" maxLength={1000} rows={2} className="w-full px-3 py-2 bg-[#0b1020] border border-white/10 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors resize-none" placeholder="Draft copy, hashtags, or instructions" />
        </div>
